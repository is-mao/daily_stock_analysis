    ...
    "dashboard": {
        ...
        "data_perspective": {
            ...
            "chanlun_analysis": {
                ...
                "chanlun_score": 0-100,
                "zhongshu_count": 中枢数量,
                "buy_points_count": 买点数量,
                "sell_points_count": 卖点数量,
                "has_beichi": true/false,
                "beichi_type": "背驰类型或无",
                "chanlun_summary": "缠论分析一句话总结"
            }
        },
...
6. **🌊 缠论分析必须包含**：data_perspective.chanlun_analysis字段必须完整填写，包含所有缠论分析结果

**重要提醒**：如果输入数据中包含缠论分析信息，必须在JSON输出的data_perspective.chanlun_analysis中完整体现，不得遗漏！"""
...
"""
        ...
        # 添加缠论分析结果
        if 'chanlun_analysis' in context:
            chanlun = context['chanlun_analysis']
            trend_emoji = {"上涨": "📈", "下跌": "📉", "盘整": "📊"}.get(chanlun.get('trend_type', ''), "📊")

            recent_buy_points = chanlun.get('recent_buy_points', [])
            buy_points_text = ""
            if recent_buy_points:
                buy_points_text = "\n**最近买点**：\n" + "\n".join(
                    [
                        f"- {p['type']}@{p['price']:.2f}元 (置信度{p['confidence']:.1f}) - {p['reason']}"
                        for p in recent_buy_points
                    ]
                )
...
| 指标 | 数值 | 说明 |
|------|------|------|
| 走势类型 | {trend_emoji}{chanlun.get('trend_type', '未知')} | 当前走势的缠论分类 |
| 缠论评分 | {chanlun.get('chanlun_score', 0):.0f}/100 | 综合缠论信号强度 |
| 分型数量 | {chanlun.get('fenxing_count', 0)}个 | 识别的顶底分型 |
| 笔数量 | {chanlun.get('bi_count', 0)}笔 | 构造的上涨下跌笔 |
| 中枢数量 | {chanlun.get('zhongshu_count', 0)}个 | 价格震荡中枢 |
| 买点数量 | {chanlun.get('buy_points_count', 0)}个 | 一二三类买点 |
| 卖点数量 | {chanlun.get('sell_points_count', 0)}个 | 一二三类卖点 |
| 背驰情况 | {chanlun.get('beichi_type', '无')} | 趋势转折信号 |

**缠论摘要**：{chanlun.get('summary', '缠论分析完成')}{buy_points_text}
"""
...
- **检查清单**：每项用 ✅/⚠️/❌ 标记
- **🌊 缠论分析**：必须在data_perspective.chanlun_analysis中包含完整的缠论分析结果
