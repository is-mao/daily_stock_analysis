...
class StockAnalysisPipeline:
    ...
    def process_single_stock(
        self, code: str, skip_analysis: bool = False, single_stock_notify: bool = False
...
示例:
  ...
  python main.py --stock-selection --data-source sina  # 使用新浪数据源（极速）
  ...
  python main.py --stock-selection --data-source efinance  # 使用EFinance数据源（快速）
    ...
    parser.add_argument('--stock-selection', action='store_true', help='运行每日股票精选功能')

    parser.add_argument('--selection-count', type=int, default=20, help='股票精选数量（默认20只）')
    ...
    parser.add_argument(
        '--data-source',
        type=str,
        choices=['auto', 'sina', 'tencent', 'tonghuashun', 'efinance', 'akshare', 'tushare', 'baostock', 'yfinance'],
        default='auto',
        help='指定数据源（默认自动选择）',
    )
...
def run_stock_selection(config: Config, args: argparse.Namespace) -> Optional[str]:
    """
    执行每日股票精选
    ...
    try:
        ...
        # 执行股票精选
        selected_stocks = selector.select_daily_stocks(strategy=strategy, max_stocks=max_stocks)
